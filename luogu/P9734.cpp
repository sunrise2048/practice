...
struct edge{
    ...
    ll l,c;
    edge()=default;
    edge(int v,ll l,ll c):v(v),l(l),c(c){}
};
vector<edge> e[N];
ll f[N][N],g[N][N];
bool fl[N];
struct ques{
    ...
    int id;
    ques()=default;
    ques(int v,ll t,int id):v(v),t(t),id(id){}
};
vector<ques> qs[N];
vector<ll> ans;
ll d[N];
...
int main(){
    ...
    for(int i=1;i<=m;++i){
        ...
        ll l,c;
        fin>>u>>v>>l>>c;
        e[u].emplace_back(v,l,c);
        e[v].emplace_back(u,l,c);
    }
    ...
    for(int i=0;i<q;++i){
        ...
        ll t;
        fin>>u>>v>>t;
        qs[u].emplace_back(v,t,i);
    }
    for(int i=0;i<n;++i){
        sort(qs[i].begin(),qs[i].end(),[](const ques& x,const ques& y){
            return x.t<y.t;
        });
        ll nt=s;
        ...
        while(!qs[i].empty()){
            ...
            while(no!=-1){
                ...
                if(!ff[no]){
                    ff[no]=1;
                    for(int k=0;k<n;++k){
                        as[k]=min(as[k],s+f[no][k]);
                    }
                }
                fl[no]=1;
                for(auto [v,l,c]:e[no]){
                    if(d[no]+l>=d[v])continue;
                    if(d[no]+nt>c-l){
                        nxt=max(nxt,c-l-d[no]);
                        continue;
                    }
                    d[v]=d[no]+l;
                }
                ...
                for(int j=0;j<n;++j){
                    if(fl[j])continue;
                    if(no==-1||d[no]>d[j])no=j;
                }
            }
            while(!qs[i].empty()&&qs[i].back().t>nxt){
                auto [v,t,id]=qs[i].back();
                qs[i].pop_back();
                ans[id]=as[v];
                if(ans[id]>=s)ans[id]-=t;
            }
            nt=nxt;
        }
    }
