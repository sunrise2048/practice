...
int main() {
    ...
    while (T--) {
        ...
        bool ok = 1;
        for (int i = 1; i <= n; ++i) if (q[i] < 0) ok = 0;
        if (!ok) { cout << "No\n"; continue; }
        ...
        for (int id : dif) bel[bcj.find(U[id])].push_back(id);
        ...
        for (int rt = 1; rt <= n; ++rt) if (!bel[rt].empty()) {
            ++tim;
            vector<int> vs;
            int sum = 0;
            for (int id : bel[rt]) {
            ...
            if (sum > 0) {
                ...
                gv.push_back(vs), ge.push_back(bel[rt]);
            } else {
                ...
                for (int v : vs) if (p[v] || q[v] || in[v] != 1 || out[v] != 1) ok = 0;
                cyc.push_back(vs);
            }
        }
        ...
        vector<int> ans; ans.reserve(3 * m);
        ...
        auto visit = [&](int v) {
            if (src[v] && !done[v]) done[v] = 1, detour(v);
        };

        for (int r : rt) if (!ing[r] && !done[r]) done[r] = 1, detour(r);

        vector<int> pos(n + 1, -1);

        for (int t = 0; t < (int)gv.size(); ++t) {
            auto &vs = gv[t];
            auto &es = ge[t];
            ...
            while (!st.empty()) {
                int x = st.back().first;
                while (it[x] < (int)g[x].size() && used[g[x][it[x]]]) ++it[x];
                if (it[x] == (int)g[x].size()) {
